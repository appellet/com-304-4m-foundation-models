...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        # getting the input embedding layer on which we apply the tokens later to have embedding that have dim dimension 
        self.input_embedding = nn.Embedding(vocab_size,dim)  # TODO: Define the input embedding layer

        self.positional_embedding = nn.Parameter(torch.randn((max_seq_len, dim),dtype= torch.float) ) #  Define the learnable positional
        # embedding
        ...
        self.out_norm =  nn.LayerNorm(dim ,bias = use_bias) # Define the output layer normalization. Use the LayerNorm class 
        # defined in 
        # modeling/transformer_layers.py
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        x = self.input_embedding(x) # [B,L,D] D = dim 
        ...
        # the mask matrix will have a shape [1, L, L] where we mask all attentions between a query and the future tokens keys (ensure autoregressive 
        # generation ) ( if False we will replace by -inf in attention block)
        mask = torch.tril(torch.ones((L,L), device = self.device,dtype = torch.bool)).unsqueeze(0)  # shape [1 , L ,L  ]
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, padding_idx: int = -100) -> torch.Tensor:
        ...
        loss = nn.CrossEntropyLoss(ignore_index = padding_idx) # ignore padding token index
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            logits = self.forward_model(current_tokens) # [B,L,vocab_size]
            ...
            logits = logits[-1,-1:,:]  # shape : [L,vocab_size]  get last tokens in the last batch
            sample, _ = sample_tokens(logits, temp, top_k, top_p)
            
            # Concatenate the new token to the current_tokens sequence
            current_tokens = torch.cat([current_tokens,sample.unsqueeze(1)], dim=1)
            
            # Break if the end-of-sequence token is generated
            if (eos_idx is not None and  sample.item() == eos_idx):
                break
