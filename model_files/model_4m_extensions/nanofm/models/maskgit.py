...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.input_embedding =  nn.Embedding(vocab_size,dim) # Define the input embedding layer
        self.positional_embedding = nn.Parameter(torch.randn((seq_len,dim), dtype = torch.float)) # Define the learnable positional embedding
        self.mask_token = nn.Parameter(torch.randn(dim,dtype= torch.float)) # Define the learnable mask token 
        # (a token that will indicate to the model that it needs to fill this missing part sort of unknown token)
        ...
        self.out_norm = nn.LayerNorm(dim ,bias = use_bias) #  Define the output layer normalization.
        
        self.to_logits = nn.Linear(dim, vocab_size, bias = False)   # Define the output projection layer
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        x[mask] = self.mask_token  # (replace the masked tokens with the mask_token embedding )

        # TODO: Add the positional embeddings to the tokens
        # as we have sequence length is L so we need to L positions  for the adding procedure adding [L,D] + [B,L,D] broadcasted automatically
        x = self.positional_embedding[:L,:] + x  # output is  [B, L , D (dim) ] 

        # TODO: Forward pass through Transformer trunk
        # Hint: No causal mask is needed here, since we are using full self-attention.
        x = self.trunk(x) # set the mask to none
        ...
        x = self.out_norm(x)
        x = self.to_logits(x) # x is of shape  [B,L,vocab_size]
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        # Generate random numbers of tokens to mask for each sample (between 1 and L)
        num_masked = torch.randint(1, L+1 , (B,), device=seq.device)
        # Create random permutation of indices for each sample
        perm = torch.randn((B, L),dtype = torch.float, device=seq.device).argsort(dim=1)
        # Create mask by comparing position indices with number to mask 
        random_mask = perm < num_masked.unsqueeze(1)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        # Anywhere in the targets tensor where the value is ignore_index , ignore it when computing loss.
        B,L,vocab_size = logits.shape
        loss = nn.CrossEntropyLoss(ignore_index =ignore_index ) # ignore padding token index
        lossv = loss(logits.reshape(B*L,vocab_size), target_seq.reshape(B*L))
        return lossv
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            logits = self.forward_model(seq, mask) # Shape: [1, L, vocab_size]
            
            # TODO: Get the indices of masked tokens. Shape: [M,] (M = number of masked tokens)
            masked_indices = torch.nonzero(mask[0]).squeeze(1) # nonrezo give us a unsqueezed tensor on dim 1 , shape : [M] 

            # TODO: Get the logits for the `masked_indices` positions. Shape: [M, vocab_size]
            masked_logits = logits[0, masked_indices, :] # shape of [M, vocab_size]
            ...
            confidence = torch.max(masked_logits, dim=1).values  # Shape: [M,] get the maximum logit from each masked position 
            ...
            k_indices = confidence.argsort(dim=0, descending=True)[:k] # get indices of the tokens to unmask from the our masked tokens array [k,]
            selected_positions = masked_indices[k_indices] # from the indices take their position within all tokens in the batch [k,]
            
            # TODO: Get the logits for the `selected_positions`. Shape: [k, vocab_size]
            selected_logits = logits[0,selected_positions]  # squeeze as logits [1,L,vocab_size] result : [k, vocab_size]
            ...
            samples, _ = sample_tokens(selected_logits, temp, top_k, top_p) # sampling procedure
            ...
            seq[0,selected_positions] = samples # update the samples 
            mask[0,selected_positions] = False # update the mask 
